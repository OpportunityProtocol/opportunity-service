...
async function publishVoteCensus(
  ...
  const publicKeyClaims: {
    key: string
    value: string
  }[] = accountsToPublicKeys.map((account) => {
    const hashedAddress = CensusOffChain.Anonymous.digestPublicKey(
      BigInt(account.pubKey),
      BigInt(account.pubKey),
    )
    const value = ''
  ...
  let { censusId } = await CensusOffChainApi.addCensus(
    censusName,
    adminPublicKeys,
    gwPool,
    entityWallet,
  ...
  let result = await CensusOffChainApi.addClaimBulk(
    censusId,
    publicKeyClaims,
    true,
    gwPool,
  ...
  merkleTreeOrigin = await CensusOffChainApi.publishCensus(
    censusId,
    gwPool,
    entityWallet,
  )
